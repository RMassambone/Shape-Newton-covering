...
gfortran -c -O3 -I$ALGENCAN/sources/algencan/inc/ covering-america.f90

gfortran covering-america.o modamerica.o vorintpols.o drand.o geometry.o geompack2.o \
         -L$ALGENCAN/sources/algencan/lib -L$ALGENCAN/sources/hsl/lib \
	 -L$ALGENCAN/sources/blas/lib -lalgencan -lhsl -lblas -o covering-america
...
do
  ...
  do
    echo $nballs $itrial $bestrad | timeout --preserve-status 30 ./covering-america
    export statdtris=$?
    if [ -e tabline.txt ] ; then
      sed -e 's/$/  S/' -i tabline.txt
      cat tabline.txt >> table-covering-america.txt ;
